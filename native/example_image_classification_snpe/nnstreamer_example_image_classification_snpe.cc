/**
 * @file	nnstreamer_example_image_classification_snpe.cc
 * @date	8 May 2020
 * @brief	Tensor stream example with filter using SNPE sub-plugin
 * @author	Yongjoo Ahn <yongjoo1.ahn@samsung.com>
 * @bug		No known bugs.
 *
 * NNStreamer example for image classification using SNPE
 *
 * Pipeline :
 ...
 * Get model by
 * $ cd $NNST_ROOT/bin
 * $ bash get-model.sh image-classification-snpe
 *
 ...
 * $ ./nnstreamer_example_image_classification_snpe
 */
...
#define MODEL_INPUT_HEIGHT  299
#define MODEL_INPUT_WIDTH   299

/**
 * @brief Data structure for snpe model info.
 */
typedef struct
{
  gchar *model_path; /**< snpe model file path */
...
typedef struct
{
  ...
  snpe_info_s snpe_info; /**< snpe model info */
} AppData;
...
/**
 * @brief Free data in snpe info structure.
 */
static void
_snpe_free_info (snpe_info_s * snpe_info)
{
  g_return_if_fail (snpe_info != NULL);

  if (snpe_info->model_path) {
    g_free (snpe_info->model_path);
    snpe_info->model_path = NULL;
  }

  if (snpe_info->label_path) {
    g_free (snpe_info->label_path);
    snpe_info->label_path = NULL;
  }

  if (snpe_info->labels) {
    g_list_free_full (snpe_info->labels, g_free);
    snpe_info->labels = NULL;
  }
}

/**
 * @brief Check snpe model and load labels.
 *
 * This example uses 'inception_v3' for image classification.
 */
static gboolean
_snpe_init_info (snpe_info_s * snpe_info, const gchar * path)
{
  const gchar snpe_model[] = "inception_v3_quantized.dlc";
  const gchar snpe_label[] = "imagenet_slim_labels.txt";

  FILE *fp;

  g_return_val_if_fail (snpe_info != NULL, FALSE);
  ...
  snpe_info->model_path = g_strdup_printf ("%s/%s", path, snpe_model);

  if (access (snpe_info->model_path, F_OK) != 0) {
    g_critical ("cannot find snpe model [%s]", snpe_info->model_path);
    return FALSE;
  }

  /* load labels */
  snpe_info->label_path = g_strdup_printf ("%s/%s", path, snpe_label);

  if ((fp = fopen (snpe_info->label_path, "r")) != NULL) {
    char *line = NULL;
    size_t len = 0;
    ssize_t read;
    gchar *label;
    ...
    while ((read = getline (&line, &len, fp)) != -1) {
      ...
      snpe_info->labels = g_list_append (snpe_info->labels, label);
    }
    ...
    g_critical ("cannot find snpe label [%s]", snpe_info->label_path);
    return FALSE;
  }

  snpe_info->total_labels = g_list_length (snpe_info->labels);
  _print_log ("finished to load labels, total %d", snpe_info->total_labels);
  return TRUE;
}
 ...
 */
static gchar *
_snpe_get_label (snpe_info_s * snpe_info, gint index)
{
  gint length;

  g_return_val_if_fail (snpe_info != NULL, NULL);
  g_return_val_if_fail (snpe_info->labels != NULL, NULL);

  length = g_list_length (snpe_info->labels);
  g_return_val_if_fail (index >= 0 && index < length, NULL);

  return (gchar *) g_list_nth_data (snpe_info->labels, index);
}

/**
 * @brief Update snpe label index with max score.
 * @param scores array of scores
 * @param len array length
 * @return None
 */
static void
_update_top_label_index (gfloat * scores, guint len)
{
  guint i;
  gint index = -1;
  gfloat max_score = 0;
  ...
  g_return_if_fail (len == g_app.snpe_info.total_labels);
...
_free_app_data (void)
{
  ...
  _snpe_free_info (&g_app.snpe_info);
}
  ...
  if (g_app.running) {
    ...
    for (i = 0; i < num_mems; i++) {
      ...
      if (gst_memory_map (mem, &info, GST_MAP_READ)) {
        ...
        _update_top_label_index ((gfloat *) info.data, (guint) info.size / 4);
...
_timer_update_result_cb (gpointer user_data)
{
  if (g_app.running) {
    ...
    if (g_app.current_label_index != g_app.new_label_index) {
      ...
      label = _snpe_get_label (&g_app.snpe_info, g_app.current_label_index);
...
{
  const gchar snpe_model_path[] = "./snpe_image_classification";
  ...
  _check_cond_err (_snpe_init_info (&g_app.snpe_info, snpe_model_path));
